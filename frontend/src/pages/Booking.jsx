    ...
    <div className="rounded-3xl bg-white p-8 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="mt-8 grid gap-6 md:grid-cols-2">
        <label className="block">
          ...
          <input type="date" required value={date} onChange={(e) => setDate(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 p-3" />
        </label>
        <label className="block">
          <span className="text-sm font-medium text-slate-700">Select slot</span>
        ...
        <div className="md:col-span-2">
          <button type="submit" className="rounded-full bg-slate-900 px-6 py-3 text-white hover:bg-slate-700">Confirm booking</button>
        </div>
      </form>
